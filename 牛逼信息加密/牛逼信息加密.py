...
class SysTrayIcon(object):
    ...
    def __init__(s, icon, hover_text, menu_options, on_quit, tk_window=None, default_menu_index=None,
                 window_class_name=None, NoExit=True):
        '''
        icon         需要显示的图标文件路径
        ...
        if NoExit:
            try:
                ...
                warnings.warn("你的菜单格式不对，已经换成默认", RuntimeWarning)
        ...
        try:
            ...
            a = (('****', None, pas), ('****', None, (('****', None, pas),)))
            warnings.warn("你的菜单格式不对，已经换成默认", RuntimeWarning)
            s.menu_options = s._add_ids_to_menu_options(list(a))
            bool = True
            print(p)
        ...
        # if bool:
        #     raise CaiDanError("你的菜单格式不对,以换成默认,格式: (('a', None, callback), ('b', None, (('b1', None, callback),)))")
    ...
    def refresh(s, title='', msg='', time=500):
        ...
        if os.path.isfile(s.icon):
            ...
            warnings.warn("托盘图标更换失败,已经使用默认值", RuntimeWarning)
...
class _Main:  # 调用SysTrayIcon的Demo窗口
    ...
    def main(s, root, menu=None, file="", hover_text="空气", NoExit=True):
        # tk窗口Visual Studio 2015
        s.icon = file
        s.NoExit = NoExit
        s.hover_text = hover_text
        if menu == None:
            s.caidan = (('啥也不是', None, pas), ('二级 菜单', None, (('啥也不是', None, pas),)))
        else:
            s.caidan = menu
        s.root = root
        s.root.protocol('WM_DELETE_WINDOW', s.Hidden_window)  # 点击Tk窗口关闭时直接调用s.exit，不使用默认关闭
    ...
    def show_msg(s, title='标题', msg='内容', time=500):
        try:
            ...
            raise Show_MsgError("请在最小化时弹出消息")
...
def jie(i_):
    ...
    if len(i_) % 2 == 1:
        i_ += "γ"
        for q in range(0, int(len(i_) / 2)):
            a += i_[q] + " "
        for q in range(int(len(i_) / 2), len(i_)):
            a = a.replace(" ", i_[q], 1)
        a = a.replace("γ", "")
    else:
        for q in range(0, int(len(i_) / 2)):
...
def pinyin_2_hanzi(_1):
    # 10个候选值
    result = dag(DefaultDagParams(), _1, path_num=10, log=True)
...
def start():
    if cli.get() == "加密":
        text.delete(0.0, END)
        text.insert(INSERT, jia(entry.get()))
    else:
        x1 = ""
        i = entry.get()
        b = ""
        ...
        b += jie(i[1])
        text.delete(0.0, END)
        text.insert(INSERT, pinyin_2_hanzi(x1.split("-")) + "\n" + b)
...
if __name__ == "__main__":
    ...
    label1 = Label(app, text='''需加密/解密:''')
    label1.place(x=11, y=10)

    button1 = Button(app, text='开始加密/解密', command=start)
    button1.place(x=250, y=40)
    ...
    text = Text(app, width=40, height=5)
    text.place(x=90, y=80)

    label1 = Label(app, text='''加密/解密结果''')
    label1.place(x=7, y=80)

    app.iconbitmap('密码.ico')
    minimum.main(app, file="密码.ico",hover_text="牛逼专用加密工具！")